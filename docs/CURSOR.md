...
- Decision brief: plain `.txt` via `src/lib/report.ts`, not HTML
...
`/admin` edits master presets/pricing/transit in **localStorage** only — no auth. Do not treat it as a multi-tenant console.
